...
export default function Edit() {
    ...
    React.useEffect(() => {
        setTestimoni(JSON.parse(localStorage.getItem('testimoni')));
    }, [])

    const deleteTestimoni = async () => {
        setIsLoadingDelete(true);
        await deleteDoc(doc(db, "testimonies", testimoni.id))
            .then(() => {
                Swal.fire({
                    position: 'top-end',
                    icon: 'success',
                    title: 'Success deleting Testimoni',
    ...
    const editTestimoni = async () => {
        setIsLoading(true);
        await updateDoc(doc(db, "testimonies", testimoni.id), {
            nama,
            prodi,
                ...
                <div className="w-full flex justify-start flex-wrap">
                    <form onSubmit={handleSubmit} className="w-full bg-white rounded-lg p-5 mb-5">
                        <div className="mb-5">
                            ...
                            <InputAdmin defaultValue={testimoni.nama} onChange={(e) => setNama(e.target.value)} type='text' required className='w-full' placeholder='title' beginningIcon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        ...
                        <div className="mb-5">
                            ...
                            <InputAdmin defaultValue={testimoni.prodi} onChange={(e) => setProdi(e.target.value)} type='text' required className='w-full' placeholder='prodi' beginningIcon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        ...
                        <div className="mb-5">
                            ...
                            <InputAdmin  defaultValue={testimoni.image_link} onChange={(e) => setImageLink(e.target.value)} type='text' required className='w-full' placeholder='link' beginningIcon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        ...
                        <div className="mb-5">
                            ...
                            <InputAdmin defaultValue={testimoni.description} onChange={(e) => setDeskripsi(e.target.value)} type='text' required className='w-full' placeholder='deskripsi' beginningIcon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
